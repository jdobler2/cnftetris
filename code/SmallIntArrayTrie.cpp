...
SmallIntArrayTrie::SmallIntArrayTrie() {
   myLayer = 0;
   this->setMaxNodeID(2);
}

SmallIntArrayTrie::SmallIntArrayTrie(int newLayer) {
   myLayer = newLayer;
   this->setMaxNodeID(2);
}

void SmallIntArrayTrie::setMaxNodeID(int mNID) {
   maxNodeID = mNID; 
   bits.resize(3, false);
...
int SmallIntArrayTrie::insert(const vector<int>& box) {   
 ...
 //     std::cout << "Returning in 1" << std::endl;
  ...
  //       std::cout << "Returning false in 2" << std::endl;
      ...
      if (branches.at(index)==NULL) {
         if (_LAYERS - myLayer - 1 == GIAT_SWITCH) { //#remaining levels == the switch
            //branches.at(index).reset(new GroupedIntArrayTrie(myLayer+1, GIAT_SWITCH));
            branches.at(index).reset(new SIMD_GIATrie(myLayer+1));
         }
         else if (index==0 && 
                  myLayer + 2 < _LAYERS - GIAT_SWITCH &&
                  box.at(myLayer+1) == 1) 
         {
            int layersSkipped = 1;
            int giatFlag = 1;
            for (int i = myLayer + 2; i < _LAYERS - GIAT_SWITCH; i++) {
               if (box.at(i)==1) {
                  layersSkipped++;
               }
               else {
            ...
            branches.at(index).reset(new ZipIntTrie(myLayer+1, layersSkipped));
            ...
            if (giatFlag) {
               ...
               temp.reset(new SmallIntArrayTrie(myLayer+1+layersSkipped));
            }
            branches.at(index)->append(temp, 0);
            //todo: insert now?
         }
         else {//cannot zip or go to GIAT.
            branches.at(index).reset(new SmallIntArrayTrie(myLayer+1));
         }
      }
...
bool SmallIntArrayTrie::contains(const vector<int>& box, vector<int>& output) {
...
//DEPRECATED
bool SmallIntArrayTrie::contains(const vector<int>& box,
                                 int currentLayer, 
                                 vector<int>& output) 
{
...
//todo: better way?
void SmallIntArrayTrie::apply(trie_actor* actor) {
...
void SmallIntArrayTrie::getAllContainingBoxesB(
      ...
      if (bits[index] == 1) { //<-- this check is redundant
         ...
         if (branches.at(index)!=NULL) {
            branches.at(index)->getAllContainingBoxesB(
                                   ...
                                   prevWord, 
                                   ++currentDepth);
            currentDepth--; //TODO: not sure if nec.
         }
      }//end if(bits)
...
bool SmallIntArrayTrie::remainingInputEmpty(const vector<int>& input, int start) {
...
bool SmallIntArrayTrie::isLastInput(const vector<int>& input, int start) {
   if (input.at(start) < 0) {
      return true;
   }
   return false;
...
std::string SmallIntArrayTrie::getType() {
   return "SmallIntArrayTrie";
}

bool SmallIntArrayTrie::append(std::shared_ptr<GenericIntTrie> tr, int index) {
   branches.at(index) = tr;
   return true;
}

std::shared_ptr<GenericIntTrie> SmallIntArrayTrie::getChild(int index) {
