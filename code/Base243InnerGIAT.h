...
#ifndef BASE_243_INNER_GROUPED_INT_ARRAY_TRIE_H_
#define BASE_243_INNER_GROUPED_INT_ARRAY_TRIE_H_

#include "Base243GIAT.h"

class Base243InnerGIAT : public Base243GIAT {
 ...
 public:
   Base243InnerGIAT();
   Base243InnerGIAT(int newLayer);
   virtual ~Base243InnerGIAT() {
  ...
  // void apply(trie_actor* actor); //TODO!
   ...
   Base243GIAT* get243Child(int index) {
      return (Base243GIAT*) branches.at(index).get();
   }
   
   void add243Children(std::vector<std::vector<Base243GIAT*> >& lists, int index, int& remaining);
